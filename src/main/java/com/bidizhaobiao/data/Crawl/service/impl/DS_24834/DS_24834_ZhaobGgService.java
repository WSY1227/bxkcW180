package com.bidizhaobiao.data.Crawl.service.impl.DS_24834;
 ...
 * 原网站：http://swj.cbs.gov.cn/tzgg/
 * 主页：http://swj.cbs.gov.cn
 **/
@Service
public class DS_24834_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://swj.cbs.gov.cn/tzgg/index.html";
    public String baseUrl = "http://swj.cbs.gov.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24834";
    // 网站名称
    public String sourceName = "白山市商务局";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("/index")) {
                ...
                Elements listElement = doc.select(".zyright2>ul>li:has(a)");
                if (listElement.size() > 0) {
                    String key = "询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.news1").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.news2").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        contentElement.select("div.news3").remove();
                        contentElement.select("script").remove();
                        contentElement.select("style").remove();
                        content = contentElement.outerHtml();
                    } else if (url.contains(".doc") || url.contains(".pdf") || url.contains(".zip") || url.contains(".xls")) {
                        content = "<div点击查看公告内容：<a href='" + url + "'>" + branch.getTitle() + "</a></div>";
