package com.bidizhaobiao.data.Crawl.service.impl.XX8374;
 ...
 * 原网站：http://www.celad.org.cn/List.aspx?ColumnID=17
 * 主页：http://www.celad.org.cn
 **/
@Service
public class XX8374_1_ZhongbXxService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.celad.org.cn/List.aspx?ColumnID=17";
    public String baseUrl = "http://www.celad.org.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "XX8374-1";
    // 网站名称
    public String sourceName = "大别山干部学院";
    // 信息源
    ...
    // 设置城市
    public String city = "信阳";
    // 设置县
    public String district = "新县";
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.equals(listUrl)) {
                ...
                Elements listElement = doc.select("ul.listuls>li:has(a)");
                if (listElement.size() > 0) {
                    for (Element element : listElement) {
                        ...
                        String date = "";
                        Matcher dateMat = datePat.matcher(element.select(".pubTime").text());
                        if (dateMat.find()) {
                ...
                Element nextPage = doc.select(".anpager a:contains(>)").first();
                if (nextPage != null && nextPage.attr("href").contains("__doPostBack") && serviceContext.isNeedCrawl()) {
                    serviceContext.setPageNum(serviceContext.getPageNum() + 1);
                    Map<String, Object> params = new HashMap<>();
                    params.put("ColumnID", "17");
                    params.put("__VIEWSTATE", doc.select("#__VIEWSTATE").attr("value"));
                    params.put("__EVENTTARGET", "ctl00$ctl00$CPHcenter$CPHright$AspNetPager1");
                    params.put("__EVENTARGUMENT", "" + serviceContext.getPageNum());
                    params.put("__EVENTVALIDATION", doc.select("#__EVENTVALIDATION").attr("value"));
                    params.put("ctl00$ctl00$TBKeyword", "");
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.decenter").first();
                    if (contentElement != null) {
                        ...
                        contentElement.select("div.pages-date").remove();
                        contentElement.select("div.qianyitiao").remove();
                        contentElement.select("div.houyitiao").remove();
