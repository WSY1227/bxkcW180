package com.bidizhaobiao.data.Crawl.service.impl.XX8390;
...
/**
 * 程序员：徐文帅 日期：2023-01-17
 * 原网站：http://www.ahfyez.com/listarticle-25/
 * 主页：http://www.ahfyez.com
 **/
@Service
public class XX8390_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.ahfyez.com/listarticle-25/list-1.html";
    public String baseUrl = "http://www.ahfyez.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "XX8390-1";
    // 网站名称
    public String sourceName = "阜阳二中";
    // 信息源
    ...
    // 设置城市
    public String city = "阜阳";
    // 设置县
    public String district = "颍泉";
        ...
        try {
            ...
            if (url.contains("/list-")) {
                ...
                if (listElement.size() > 0) {
                    String key = "服务项目、询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、择选、择优、选取、优选、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                    String[] keys = key.split("、");
                    for (Element element : listElement) {
                        Element a = element.select("a").first();
                        String link = a.attr("href").trim();
                        String id = link.substring(link.lastIndexOf("com/") + 4);
                        String detailLink = baseUrl + id;
                        String date = "";
                ...
                if (serviceContext.getPageNum() == 1) {
                    Pattern compile = Pattern.compile("/list-(\\d+).html");
                    Matcher matcher = compile.matcher(doc.select("a:contains(尾页)").first().attr("href"));
                    if (matcher.find()) {
                ...
                if (serviceContext.getPageNum() <= serviceContext.getMaxPage() && serviceContext.isNeedCrawl()) {
                    ...
                    String href = listUrl.replace("1", "" + serviceContext.getPageNum());
                ...
                if (branch != null) {
                    ...
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.con_3_title").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        contentElement.select("div.location_info").remove();
                        contentElement.select("div.info").remove();
                        contentElement.select("#prev_next").remove();
