package com.bidizhaobiao.data.Crawl.service.impl.DX013357;
 ...
 * 原网站：http://www.ybsjjtgs.com/?catid=47
 * 主页：http://www.ybsjjtgs.com
 **/
@Service
public class DX013357_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.ybsjjtgs.com/index.php?catid=47&page=13";
    public String baseUrl = "http://www.ybsjjtgs.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "DX013357-1";
    // 网站名称
    public String sourceName = "宜宾三江投资建设集团有限公司";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("&page=")) {
                ...
                Elements listElement = doc.select(".middle-box>.list-content:has(a)");
                if (listElement.size() > 0) {
                    String key = "征集、参选、选聘、招选、市场项目、询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                ...
                if (branch != null) {
                    ...
                    String date = "";
                    String content = "";
                    Element contentElement = doc.select("div.middle-content").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.top-title").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        Element timeElement = contentElement.select("div.news-time").first();
                        if (timeElement != null) {
                        ...
                        if (date!=null) {
                            if (Integer.parseInt(date.substring(0,4))<2016) {
                                Matcher dateMat = datePat.matcher(contentElement.select(".middle-text").text());
                                while (dateMat.find()) {
                                    date = dateMat.group(1);
                                    date += dateMat.group(3).length() == 2 ? "-" + dateMat.group(3) : "-0" + dateMat.group(3);
                                    date += dateMat.group(5).length() == 2 ? "-" + dateMat.group(5) : "-0" + dateMat.group(5);
                        ...
                        contentElement.select("div.jiange30").remove();
                        contentElement.select("div.listpage").remove();
