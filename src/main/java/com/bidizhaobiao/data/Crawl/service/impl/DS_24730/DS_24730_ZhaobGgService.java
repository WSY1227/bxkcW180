package com.bidizhaobiao.data.Crawl.service.impl.DS_24730;
 ...
 * 原网站：http://www.smszx.gov.cn/ggl/
 * 主页：http://www.smszx.gov.cn
 **/
@Service
public class DS_24730_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.smszx.gov.cn/fjdzapp/search?channelid=474554&templet=docs.jsp&sortfield=-DOCORDERPRI,-docreltime&classsql=chnlid=9853&page=1&prepWage=20";
    public String baseUrl = "http://www.smszx.gov.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24730";
    // 网站名称
    public String sourceName = "三明市政协";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            Thread.sleep(500);
            if (url.contains("docs.jsp")) {
                JSONObject doc = JSON.parseObject(page.getRawText());
                JSONArray listJSONArray = doc.getJSONArray("docs");
                if (listJSONArray.size() > 0) {
                    ...
                    for (int i = 0; i < listJSONArray.size(); i++) {
                        ...
                        String link = notList.getString("docpuburl").trim();
                        String id = link.substring(link.lastIndexOf("/") + 1);
                        String detailLink = link;
                        String date = "";
                        Matcher dateMat = datePat.matcher(notList.getString("docreltime"));
                        if (dateMat.find()) {
                        ...
                        String title = notList.getString("doctitle").trim();
                ...
                if (pageNum < pageCount && serviceContext.isNeedCrawl()) {
                    ...
                    String href = "http://www.smszx.gov.cn/fjdzapp/search?channelid=474554&templet=docs.jsp&sortfield=-DOCORDERPRI,-docreltime&classsql=chnlid=9853&page=" + pageNum + "&prepWage=20";
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.xl_main.mar-T15.mar-B20").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.n-tit").first();
                        if (titleElement != null) {
                            titleElement.select("h5").remove();
                            title = titleElement.text().trim();
                        }
                        Elements file = contentElement.select(".xl_list.mar-T5 a");
                        if (file .size()==0) {
                            file.remove();
                            contentElement.select(".fjxz_tit").remove();
                        }
                        contentElement.select("div.xl_list.mar-T5").remove();
                        contentElement.select("div.xglj_tit").remove();
                        contentElement.select("div.xl_btn_box.clearflx").remove();
