package com.bidizhaobiao.data.Crawl.service.impl.XX8427;
 ...
 * 原网站：http://www.zzvcct.com/Html/List/List_66.html
 * 主页：http://www.zzvcct.com
 **/
@Service
public class XX8427_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.zzvcct.com/Html/List/List_66.html";
    public String baseUrl = "http://www.zzvcct.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "XX8427-1";
    // 网站名称
    public String sourceName = "郑州商贸旅游职业学院";
    // 信息源
    ...
    public void startCrawl(int ThreadNum, int crawlType) {
        ...
        try {
            ...
            Thread.sleep(500);
            if (url.contains("/List_66")) {
                Document doc = Jsoup.parse(page.getRawText());
                Elements listElement = doc.select(".news-list>.news-item:has(a)");
                if (listElement.size() > 0) {
                    String key = "询标、交易、机构、废旧、废置、处置、报废、供应商、承销商、服务商、择选、择优、选取、优选、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                    ...
                    for (Element element : listElement) {
                        ...
                        String date = "";
                        Matcher dateMat = datePat.matcher(element.select(".month").text()+"-"+element.select(".day").text());
                        if (dateMat.find()) {
                ...
                Element nextPage = doc.select(".page a:contains(下一页)").first();
                if (nextPage != null && nextPage.attr("href").contains("/List_66") && serviceContext.isNeedCrawl()) {
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.news_content").first();
                    if (contentElement != null) {
                        ...
                        contentElement.select("div.attr.tc").remove();
                        contentElement.select(".bomNav").remove();
