package com.bidizhaobiao.data.Crawl.service.impl.DS_24912;
 ...
 * 原网站：http://lyj.lanzhou.gov.cn/col/col4345/index.html?uid=16907&pageNum=1
 * 主页：http://lyj.lanzhou.gov.cn/
 **/
@Service
public class DS_24912_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://lyj.lanzhou.gov.cn/module/web/jpage/dataproxy.jsp?startrecord=1&endrecord=45&perpage=15";
    public String baseUrl = "http://lyj.lanzhou.gov.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24912-1";
    // 网站名称
    public String sourceName = "兰州市林业局";
    // 信息源
    public String infoSource = "政府采购";
    // 设置地区
    public String area = "西北";
    // 设置省份
    public String province = "甘肃";
    // 设置城市
    public String city = "兰州";
    // 设置县
    ...
    public Request getListRequest(int startrecord, int endrecord) {
        Request request = new Request(listUrl.replace("startrecord=1", "startrecord=" + startrecord).replace("endrecord=45", "endrecord=" + endrecord))
        ...
        params.put("webid", "40");
        params.put("path", "/");
        params.put("columnid", "4345");
        params.put("sourceContentType", "1");
        params.put("unitid", "16907");
        params.put("webname", "兰州市林业局");
    ...
    public void startCrawl(int ThreadNum, int crawlType) {
        ...
        spider.addRequest(getListRequest(1, 45));
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("&perpage=")) {
                Document doc = Jsoup.parse("<table>" + page.getRawText().replace("<![CDATA[", "").replace("]]>", "") + "</table>");
                Elements listElement = doc.select("tr:has(a)");
                if (listElement.size() > 0) {
                    String key = "询标、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                ...
                if (serviceContext.getPageNum() == 1) {
                    int maxPageNum = Integer.parseInt(doc.select("totalpage").text());
                    serviceContext.setMaxPage(maxPageNum % 3 == 0 ? maxPageNum / 3 : maxPageNum / 3 + 1);
                }
                if (serviceContext.getPageNum() < serviceContext.getMaxPage() && serviceContext.isNeedCrawl()) {
                    serviceContext.setPageNum(serviceContext.getPageNum() + 1);
                    int startrecord = (serviceContext.getPageNum() - 1) * 45 + 1;
                    page.addTargetRequest(getListRequest(startrecord, startrecord + 44));
                }
            } else {
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select(".bian2 table:has(.title)").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("td.title").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        Elements tr = contentElement.select("tr");
                        tr.get(1).remove();
                        tr.last().remove();
