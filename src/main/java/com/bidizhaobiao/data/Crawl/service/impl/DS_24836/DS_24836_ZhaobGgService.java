package com.bidizhaobiao.data.Crawl.service.impl.DS_24836;
 ...
 * 原网站：http://xgswj.xiaogan.gov.cn/tzgg/index.jhtml
 * 主页：http://xgswj.xiaogan.gov.cn
 **/
@Service
public class DS_24836_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://xgswj.xiaogan.gov.cn/tzgg/index.jhtml";
    public String baseUrl = "http://xgswj.xiaogan.gov.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24836";
    // 网站名称
    public String sourceName = "孝感市商务局";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("/index")) {
                ...
                Elements listElement = doc.select("#bodyTab21>ul>li:has(a)");
                if (listElement.size() > 0) {
                    String key = "招商、询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                ...
                if (nextPage != null && nextPage.attr("onclick").contains("index_") && serviceContext.isNeedCrawl()) {
                    String href = url.substring(0, url.lastIndexOf("/") + 1) + nextPage.attr("onclick").trim().replaceAll(".*'(.*?)'.*", "$1");
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.readcon").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.title").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        contentElement.select("#textaudio1").remove();
                        contentElement.select(".tools").remove();
                        contentElement.select("div:has(#articleQRCode)").remove();
