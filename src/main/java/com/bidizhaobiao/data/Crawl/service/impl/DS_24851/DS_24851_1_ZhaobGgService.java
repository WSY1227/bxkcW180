package com.bidizhaobiao.data.Crawl.service.impl.DS_24851;
 ...
 * 原网站：http://www.sciep.com/node_302208.htm
 * 主页：http://www.sciep.com
 **/
@Service
public class DS_24851_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.sciep.com/node_302208.htm";
    public String baseUrl = "http://www.sciep.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24851-1";
    // 网站名称
    public String sourceName = "深圳国际人才交流中心";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            Thread.sleep(500);
            if (url.contains("/node_302208")) {
                Document doc = Jsoup.parse(page.getRawText());
                Elements listElement = doc.select(".col-xs-12.col-sm-12.col-md-6.col-lg-4.news_box>a");
                if (listElement.size() > 0) {
                    String key = "询标、废旧、废置、处置、报废、供应商、承销商、服务商、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                    ...
                    for (Element a : listElement) {
                        ...
                        String date = "";
                        Matcher dateMat = datePat.matcher(a.select(".month").text() + "." + a.select(".date").text());
                        if (dateMat.find()) {
                        ...
                        String title = a.select(".xwzx_title.title1").text().trim();
                ...
                Element nextPage = doc.select(".page-next a").first();
                if (nextPage != null && nextPage.attr("href").contains("/node_302208") && serviceContext.isNeedCrawl()) {
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.deta_box.box_xq").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.biaoti").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        contentElement.select("div.deta_title").remove();
                        contentElement.select("div.fenxiangs").remove();
                        contentElement.select("div#canvas").parents().remove();
