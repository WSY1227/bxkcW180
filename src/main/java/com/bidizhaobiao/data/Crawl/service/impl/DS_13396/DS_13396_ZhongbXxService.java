package com.bidizhaobiao.data.Crawl.service.impl.DS_13396;
 ...
 * 程序员：许广衡
 * 日期：2020-06-11
 * 原网站：http://www.hbhbgczj.com/news/class/?114.html
 * 主页：http://www.hbhbgczj.com
 **/
@Service
public class DS_13396_ZhongbXxService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.hbhbgczj.com/news/class/index.php?page=1&catid=114&myord=uptime&myshownums=&showtj=&showdate=&author=&key=";
    public String baseUrl = "http://www.hbhbgczj.com";
    // 抓取网站的相关配置，包括：编码、抓取间隔、重试次数等
    public Site site = Site.me().setCycleRetryTimes(2).setTimeOut(30000).setSleepTime(20);

    // 网站编号
    public String sourceNum = "13396";
    // 网站名称
    public String sourceName = "湖北慧博工程造价咨询有限公司";
    // 信息源
        ...
        try {
            ...
            if (url.contains("page")) {
                if (serviceContext.getPageNum() == 1) {
                    String max = doc.select("div[id=showpages]").select("option").last().text().trim().replaceAll("[\u00a0\u1680\u180e\u2000-\u200a\u2028\u2029\u202f\u205f\u3000\ufeff\\s+]+", "");
                    max = max.replaceAll("[^0-9]", "");
                    int maxPage = Integer.parseInt(max);
                    serviceContext.setMaxPage(maxPage);
                }
                Element conTag = doc.select("div[id=newsquery]").first();
                ...
                if (eachTags.size() > 0) {
                    for (Element eachTag : eachTags) {
                      ...
                      if (!CheckProclamationUtil.isProclamationValuable(title))  {
                        ...
                        if (id.contains("../..")) {
                            id = id.substring(id.indexOf("../..") + 5);
                        } else if (id.contains("../")) {
                            id = id.substring(id.indexOf("../") + 2);
                        }
                        String link = "http://www.hbhbgczj.com" + id;
                ...
                map.remove(url);//清除冗余
                String path = "http://www.hbhbgczj.com";
                ...
                String content = "";
                String title = "";
                Element conTag = doc.select("div[id=newscontent]").first();
                if (conTag != null) {
                    Element titleTag = conTag.select("div.newstitle").first();
                    conTag.select("div.info").remove();
