package com.bidizhaobiao.data.Crawl.service.impl.SJ_16389;
 ...
 * 程序员： 陈广艺
 * 日期：2021-06-10
 * 原网站：http://dzys.yn.gov.cn/gggs/index.html
 * 主页：http://dzys.yn.gov.cn/
 **/

@Service("SJ_16389_ZhongbXxService")
public class SJ_16389_ZhongbXxService extends SpiderService implements PageProcessor {

    public Spider spider = null;

    public String listUrl = "http://dzys.yn.gov.cn/gggs/index.html";
    public String baseUrl = "http://dzys.yn.gov.cn/";
    public Pattern dateRex = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "16389";
    // 网站名称
    public String sourceName = "云南滇中引水";
    // 信息源
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("index")) {
                Elements listElement = doc.select("div.div_list_body tr:has(a)");
                if (listElement.size() > 0) {
                    for (Element element : listElement) {
                        Element a = element.select("a").first();
                        String link = a.attr("href").trim().replace("./","http://dzys.yn.gov.cn/gggs/");
                        ...
                        String date = "";
                        Matcher dateMat = dateRex.matcher(element.select("td[width=\"15%\"]").text());
                        if (dateMat.find()) {
                ...
                //翻页
                if (listElement.size() == 25 && serviceContext.isNeedCrawl()) {
                ...
                if (branch != null) {
                    ...
                    Element contentElement = doc.select("div.div_article_main").first();
                    if (contentElement != null) {
                        contentElement.select("div.div_article_main p.source").remove();
                        Element titleElement = contentElement.select("div.div_article_title").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }

                        contentElement.select("div.art_info").remove();
                        contentElement.select("div.cont_next").remove();
