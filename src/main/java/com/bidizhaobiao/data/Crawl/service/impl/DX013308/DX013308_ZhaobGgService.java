package com.bidizhaobiao.data.Crawl.service.impl.DX013308;
 ...
 * 原网站：https://www.szcgc.com/companynews.aspx
 * 主页：https://www.szcgc.com/
 **/
@Service
public class DX013308_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "https://www.szcgc.com/companynews.aspx";
    public String baseUrl = "https://www.szcgc.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "DX013308";
    // 网站名称
    public String sourceName = "深圳担保集团有限公司";
    // 信息源
    ...
    public Request getListRequest(Map<String, Object> params) {
        Request request = new Request(listUrl);
        request.setMethod(HttpConstant.Method.POST);
        request.addHeader("Content-Type", "application/x-www-form-urlencoded");
        request.addHeader("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9");
        request.addHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36 Edg/108.0.1462.76");
        //设置request参数
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("/company")) {
                ...
                Elements listElement = doc.select(".NewsList>ul>li:has(a)");
                if (listElement.size() > 0) {
                    String key = "出售、询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                ...
                if (nextPage != null && nextPage.attr("href").contains("__doPostBack") && serviceContext.isNeedCrawl()) {
                    ...
                    Matcher matcher = pattern.matcher(nextPage.attr("href"));
                    Map<String, Object> params = new HashMap<>();
                    params.put("__VIEWSTATE", doc.select("#__VIEWSTATE").first().attr("value"));
                    params.put("__VIEWSTATEGENERATOR", doc.select("#__VIEWSTATEGENERATOR").first().attr("value"));
                    params.put("__EVENTTARGET", matcher.find() ? matcher.group(1) : null);
                    params.put("__EVENTARGUMENT", matcher.find() ? matcher.group(1) : null);
                    params.put("keyword", "");
                    serviceContext.setPageNum(serviceContext.getPageNum() + 1);
                    page.addTargetRequest(getListRequest(params));
                }
            } else {
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("#S_left").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("li.tilet").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        contentElement.select("div.share").remove();
                        contentElement.select("div.NS_title").remove();
