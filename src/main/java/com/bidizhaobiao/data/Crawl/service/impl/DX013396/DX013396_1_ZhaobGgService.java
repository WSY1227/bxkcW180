package com.bidizhaobiao.data.Crawl.service.impl.DX013396;
 ...
 * 原网站：http://cysw.bsjstzjt.com/list/bsjtcyswPc/18/493/auto/20/0.html
 * 主页：http://cysw.bsjstzjt.com
 **/
@Service
public class DX013396_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://cysw.bsjstzjt.com/subsiteIndex/toPage?subsiteFlag=bsjtcyswPc&subsiteId=18&newsClassId=493&pageType=auto&pageSize=20&start=1&objectId=";
    public String baseUrl = "http://cysw.bsjstzjt.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "DX013396-1";
    // 网站名称
    public String sourceName = "保山昌源水务有限责任公司";
    // 信息源
    ...
    public void startCrawl(int ThreadNum, int crawlType) {
        ...
        try {
            ...
            if (url.contains("&start=")) {
                ...
                Elements listElement = doc.select("#list>dd:has(a)");
                if (listElement.size() > 0) {
                    String key = "服务、询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                ...
                Element nextPage = doc.select("a.nextPage").first();
                if (nextPage != null && nextPage.attr("href").contains("&start=") && serviceContext.isNeedCrawl()) {
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.newsView").first();
                    if (contentElement != null) {
                        ...
                        contentElement.select("div.articleIntro").remove();
                        contentElement.select("div.share").remove();
                        contentElement.select("div.article_pageUp").remove();
