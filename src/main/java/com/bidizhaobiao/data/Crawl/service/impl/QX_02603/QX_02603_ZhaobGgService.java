package com.bidizhaobiao.data.Crawl.service.impl.QX_02603;
...
/**
 * 程序员：余林锐 日期：2021-06-03
 * 原网站：http://www.lhsl.gov.cn/zwgk/Zbgg/index.html
 * 主页：http://www.lhsl.gov.cn
 **/

@Service("QX_02603_ZhaobGgService")
public class QX_02603_ZhaobGgService extends SpiderService implements PageProcessor {

    public Spider spider = null;

    public String listUrl = "http://www.lhsl.gov.cn/zwgk/Zbgg/index.html";

    public String baseUrl = "http://www.lhsl.gov.cn";

    // 抓取网站的相关配置，包括：编码、抓取间隔、重试次数等
    Site site = Site.me().setCycleRetryTimes(3).setTimeOut(30000).setSleepTime(20);
    // 网站编号
    public String sourceNum = "02603";
    // 网站名称
    public String sourceName = "召陵区人民政府";
    // 信息源
    ...
    // 设置城市
    public String city = "漯河市";
    // 设置县
    public String district = "召陵区";
    // 设置CreateBy
    ...
    public Site getSite() {
        String cookie = "";
        for (int i = 0; i < 3; i++) {
            cookie = getCookie(listUrl);
            if (cookie != null && !cookie.equals("")) {
                break;
            }
        }
        return this.site.addHeader("Cookie", cookie).setUserAgent("Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36");
    }
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("index")) {
                ...
                if (serviceContext.getPageNum() == 1 && document.select("div.padding-big span:contains(共)").first() != null) {
                    String pageCount = document.select("div.padding-big span:contains(共)").last().text().trim();
                    pageCount = pageCount.replace("共", "").replace("页", "").trim();
                    int maxPage = Integer.valueOf(pageCount);
                    serviceContext.setMaxPage(maxPage);
                }
                Elements lis = document.select("div.padding-big > ul > li:has(a)");
                if (lis.size() > 0) {
                    for (Element li : lis) {
                        ...
                        String date = li.select("span.date").last().text().trim();
                ...
                String content = "";
                Document document = Jsoup.parse(page.getRawText().replace("&nbsp;", "").replace("&amp;", "&").replace("&ensp;", "").replace("<![CDATA[", "").replace("]]>", "").replace("&lt;", "<").replace("&gt;", ">"));
                Element contentE = document.select("div.panel-body").first();
                Elements elements = contentE.getAllElements();
                for (Element element : elements) {
                    element.removeAttr("style");
                }
                contentE.select("p.border-top-bottom").remove();
    ...
    public String getCookie(String url) {
        ...
        HttpGet httpGet = new HttpGet(url + "?security_verify_data=313932302c31303830");
        RequestConfig requestConfig = RequestConfig.custom().setRedirectsEnabled(false).setConnectTimeout(10 * 1000).setSocketTimeout(60 * 1000).build();
        ...
        try {
            ...
            for (Header header : headers) {
                if (header.getValue().contains("security_session_mid_verify")) {
