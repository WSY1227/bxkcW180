package com.bidizhaobiao.data.Crawl.service.impl.QX_19416;
 ...
 * 原网站：http://www.hrbsdeyy.org.cn/news.php
 * 主页：http://www.hrbsdeyy.org.cn
 **/
@Service
public class QX_19416_ZhongbXxService extends SpiderService implements PageProcessor {
    public Spider spider = null;
    //列表界面
    public String listUrl = "http://www.hrbsdeyy.org.cn/news.php";
    //域名
    public String baseUrl = "http://www.hrbsdeyy.org.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "19416";
    // 网站名称
    public String sourceName = "哈尔滨市第二医院";
    // 信息源
    public String infoSource = "政府采购";
    // 设置地区
    public String area = "东北";
    // 设置省份
    public String province = "黑龙江";
    // 设置城市
    public String city = "哈尔滨市";
    // 设置县
    public String district = "道外区";
    ...
    public void process(Page page) {
        ...
        try {
            ...
            Thread.sleep(500);
            if (url.contains("news.php")) {
                Document doc = Jsoup.parse(page.getRawText());
                Elements listElement = doc.select("a.clearfix");
                if (listElement.size() > 0) {
                    for (Element a : listElement) {
                        ...
                        String date = "";
                        Element timeElement = a.select(".time").first();
                        String timeP = timeElement.select("p").first().text();
                        String timeH1 = timeElement.select("h1").first().text();

                        Matcher dateMat = datePat.matcher(timeP + "-" + timeH1);
                        if (dateMat.find()) {
                        ...
                        String title = a.select(".text h1").text().trim();
                ...
                //翻页处理
                Element nextPage = doc.select(".animate-box>span>a").last();
                if (nextPage != null && nextPage.attr("href").contains("?p=") && serviceContext.isNeedCrawl()) {
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    //内容标签
                    Element contentElement = doc.select(".contents.publicity-page.party-page.view-page.clearfix>div.left").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select(".title h1").first();
                        if (titleElement != null) {
                            title = titleElement.text().trim();
                        }
                        //删除无用元素
                        contentElement.select(".title").remove();
                        contentElement.select(".console.bdsharebuttonbox.bdshare-button-style0-16").remove();
