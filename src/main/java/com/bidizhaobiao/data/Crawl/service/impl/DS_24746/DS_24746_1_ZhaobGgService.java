package com.bidizhaobiao.data.Crawl.service.impl.DS_24746;
 ...
 * 原网站：http://www.nblawyer.com/Page/PageHome/list?id=102
 * 主页：http://www.nblawyer.com
 **/
@Service
public class DS_24746_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.nblawyer.com/Page/PageHome/Page";
    public String baseUrl = "http://www.nblawyer.com";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24746-1";
    // 网站名称
    public String sourceName = "宁波市律师协会";
    // 信息源
    ...
    Site site = Site.me().setCycleRetryTimes(2).setTimeOut(30000).setSleepTime(20)
            .setUserAgent("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36 Edg/108.0.1462.76");
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.equals(listUrl)) {
                JSONObject doc = JSON.parseObject(page.getRawText()).getJSONObject("Data");
                JSONArray listJSONArray = doc.getJSONArray("Data");
                if (listJSONArray.size() > 0) {
                    String key = "询标、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、择选、择优、选取、优选、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                    String[] keys = key.split("、");
                    for (int i = 0; i < listJSONArray.size(); i++) {
                        JSONObject notList = listJSONArray.getJSONObject(i);
                        String id = notList.getString("ID");
                        String link = "http://www.nblawyer.com/Page/PageHome/Detail?id=" + id;
                        String detailLink = link;
                        String date = SpecialUtil.date2Str(notList.getDate("RELEASETIME"));
                        ...
                        String title = listJSONArray.getString(2);
                ...
                if (serviceContext.getPageNum() == 1) {
                    int num = doc.getInteger("Num") / 30;
                    if (doc.getInteger("Num") % 30 != 0) {
                        num++;
                    }
                    serviceContext.setMaxPage(num);
                }
                ...
                if (branch != null) {
                    ...
                    String content = "";
                    Element contentElement = doc.select("div.main").first();
                    if (contentElement != null) {
                        ...
                        Element titleElement = contentElement.select("div.detail-title").first();
                        if (titleElement != null) {
                            titleElement.select("p").remove();
                            title = titleElement.text().trim();
                        }
                        contentElement.select("div.bread-crumbs").remove();
                        contentElement.select("div.detail-ewm").remove();
                        contentElement.select("div.page-turning").remove();
