package com.bidizhaobiao.data.Crawl.service.impl.QX_24768;
 ...
 * 原网站：http://www.xzbg.gov.cn/zxzx_yy_4272/
 * 主页：http://www.xzbg.gov.cn
 **/
@Service
public class QX_24768_1_ZhaobGgService extends SpiderService implements PageProcessor {
    public Spider spider = null;

    public String listUrl = "http://www.xzbg.gov.cn/zxzx_yy_4272/index.html";
    public String baseUrl = "http://www.xzbg.gov.cn";
    public Pattern datePat = Pattern.compile("(\\d{4})(年|/|-|\\.)(\\d{1,2})(月|/|-|\\.)(\\d{1,2})");

    // 网站编号
    public String sourceNum = "24768-1";
    // 网站名称
    public String sourceName = "班戈县政府新闻网";
    // 信息源
    ...
    // 设置城市
    public String city = "那曲";
    // 设置县
    public String district = "班戈";
    ...
    public void process(Page page) {
        ...
        try {
            ...
            if (url.contains("/index")) {
                ...
                Elements listElement = doc.select("ul.ulListText_d>li:has(a)");
                if (listElement.size() > 0) {
                    String key = "邀请、询标、交易、机构、需求、废旧、废置、处置、报废、供应商、承销商、服务商、调研、优选、择选、择优、选取、公选、选定、摇选、摇号、摇珠、抽选、定选、定点、招标、采购、询价、询比、竞标、竞价、竞谈、竞拍、竞卖、竞买、竞投、竞租、比选、比价、竞争性、谈判、磋商、投标、邀标、议标、议价、单一来源、标段、明标、明投、出让、转让、拍卖、招租、出租、预审、发包、承包、分包、外包、开标、遴选、答疑、补遗、澄清、延期、挂牌、变更、预公告、监理、改造工程、报价、小额、零星、自采、商谈";
                    String[] keys = key.split("、");
                    for (Element element : listElement) {
                        Element a = element.select("a").first();
                        String link = a.attr("href").trim();
                        String id = link.substring(2);
                ...
                if (serviceContext.getPageNum() == 1) {
                    Pattern pattern = Pattern.compile("createPageHTML\\((\\d+),");
                    String text = doc.select(".pageBox script").first().html();
                    Matcher matcher = pattern.matcher(text);
                    if (matcher.find()) serviceContext.setMaxPage(Integer.parseInt(matcher.group(1)));
                }
                ...
                if (branch != null) {
                    ...
                    String date = "";
                    String content = "";
                    Element contentElement = doc.select("div.box").first();
                    if (contentElement != null) {
                        ...
                        Element timeElement = contentElement.select(".art_info").first();
                        if (timeElement != null) {
